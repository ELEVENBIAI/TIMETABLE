...
export function OpenReassignmentsBanner({ currentWeekStart, onJumpToWeek }: Props) {
  ...
  return (
    <div
      role="alert"
      className="flex flex-wrap items-center gap-2 rounded-md border border-status-needs-reassign/40 bg-status-needs-reassign/10 px-3 py-2 text-label"
      ...
      {weeks.map((w) => {
        ...
        return (
          <button
            ...
            className="inline-flex items-center gap-1 rounded-full border border-status-needs-reassign/50 bg-surface px-2 py-0.5 text-status-needs-reassign hover:bg-status-needs-reassign/15"
            data-testid={`open-reassignments-week-${w.weekStart}`}
          >
            <span className="numeric tabular-nums">{label}</span>
            <span className="rounded-full bg-status-needs-reassign/20 px-1.5 text-label font-semibold">
              {w.openCount}
            </span>
          </button>
        );
